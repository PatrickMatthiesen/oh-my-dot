...
CHECKSUMS_URL="https://github.com/$REPO_OWNER/$REPO_NAME/releases/download/$TAG_NAME/checksums.txt"
...
if ! curl -fsSL --max-time 30 "$CHECKSUMS_URL" -o "$TMP_DIR/checksums.txt"; then
    echo -e "${RED}Error: Failed to download $CHECKSUMS_URL${NC}"
    echo "The release must include checksums.txt generated by GoReleaser."
    exit 1
fi

EXPECTED_SHA256=$(awk -v name="$ARCHIVE_NAME" '$2 == name { print $1 }' "$TMP_DIR/checksums.txt")
if [ -z "$EXPECTED_SHA256" ]; then
    echo -e "${RED}Error: No checksum found for $ARCHIVE_NAME in checksums.txt${NC}"
    exit 1
fi
...
if command -v sha256sum > /dev/null 2>&1; then
    if ! printf '%s  %s\n' "$EXPECTED_SHA256" "$TMP_DIR/$ARCHIVE_NAME" | sha256sum -c - > /dev/null; then
    ...
    echo -e "${RED}Error: Could not find sha256sum or shasum for checksum verification${NC}"
    exit 1
fi

echo -e "${GREEN}V Checksum verified${NC}"
